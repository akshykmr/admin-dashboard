...
import data from "../../../data/master/allParking.json";
...
export default function AllParking() {
  ...
  const fetchAllParkingData = async () => {
    console.log("selectedParam :",  selectedParam,  "Value to se send as param : ", param,);
    try {
      setLoader(true);
      const headers = { token: token };
      const response = await axios.get(
        `${serverUrl}/park/fetchAll?isAssign=all&isActive=${param}`,
        {
          headers,
        }
      );
      console.log("All Parking List", response.data);
