...
export default function UpdateParking() {
  /// NOTE : NEED TO SEND IS_ASSIGNED TRUE IF BOTH CHECK IN AND CHECK OUT IS SLELECTED IF NOT SEND IT FALSE
    ...
    const fetchEventDetails = async () => {
      try {
        const response = await axios.get(
          `${serverUrl}/park/fetch/${parkingId}`
        );

        if (response.status === 200) {
          console.log(
            response.data,
            " fetched parking  data for single parking"
          );

          const commonEventData = {
            name: response.data?.details?.name,
            description: response.data?.details?.description,
            // img: [response.data?.details?.image],
            address: response.data?.details?.address,
            type: response.data?.details?.type,
            capacity: response.data?.details?.capacity,
            lat: response.data?.details?.lat,
            long: response.data?.details?.long,
            check_in_user: response.data?.check_in_user?._id,
            check_out_user: "",
          };

          //

          setParkingData(commonEventData);
  ...
  const HandleParkingSubmit = async (e) => {
    ...
    if (isFormValid) {
      try {
        ...
        if(parkingData.check_in_user && parkingData.check_out_user) {
          formData.append("is_assigned" , true)
        }
        console.log(formData, " data to be sent");

        const response = await axios.patch(
          `${serverUrl}/park/update`,
          formData,
          {
            headers,
          }
        ...
        if (response.status === 200) {
          ...
          setTimeout(() => {
            ...
            window.location.href = "/all-parking";
          }, 2000);
        }
      } catch (error) {
        setSubmitter("Submit");
