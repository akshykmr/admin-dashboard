...
import data from "../../../data/master/assignedParking.json";
import axios from "axios";
import { useNavigate } from "react-router-dom";

export default function AssignedParking() {
  const navigate = useNavigate();
  const token = localStorage.getItem("token");
  const serverUrl = process.env.REACT_APP_BASE_URL;

  const [fetchedAssignedParkingData, setFetchedAssignedParkingData] = useState();
  ...
  const fetchAssignedParkingData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${serverUrl}/park/fetchAll?isAssign=true&isActive=${param}`,
        {
          headers,
        }
      );
      console.log("Parking List with fileter param", response.data);
      setFetchedAssignedParkingData(response.data);
      setLoader(false);
    } catch (error) {
      setLoader(false);
        ...
        <Col xl={12}>
          <CardLayout>
            <Row>
              ...
              <Col xl={12}>
                <ParkingTable
                  type={"assigned-parking"}
                  thead={data?.product.thead}
                  tbody={fetchedAssignedParkingData}
                  loader={loader}
                />
                {/* <Pagination /> */}
              </Col>
